...
CREHMA.prototype.generateSignatureHeaderRequest = async function(req,kid,addHeaders,sig,hash){
	var tvp = new Date().toISOString();
	var tbs = this.buildTbsRequest(req,tvp,addHeaders,hash);
	var sv = await this.getSignatureAlgorithm(sig).sign(tbs,kid,sig);
...
CREHMA.prototype.generateSignatureHeaderResponse = async function(res,req,kid,addHeaders,sig,hash){
	var tvp = new Date().toISOString();
	...
	var tbs = tvp + "\n" + tbsWithoutTvp;
	var sv = await this.getSignatureAlgorithm(sig).sign(tbs,kid);
...
CREHMA.prototype.verifyResponse = async function(res,req){
	var result = {}
	if(res.getHeader("Signature")){
		...
		const tbs = tvp + "\n" + await this.buildTbsResponseWithoutTvp(res,primaryCacheKey,addHeaders,hash);
		...
		const signatureValidity = await signatureAlgorithm.verify(tbs,sv,kid);
...
CREHMA.prototype.verifyRequest = async function(req){
	var result = {}
...
CREHMA.prototype.buildTbsResponseWithoutTvp = async function(res,primaryCacheKey,addHeaders,hash){
	...
	if (res.getBody()){
		tbsWithoutTvp += await this.getHashAlgorithm(hash).generateHash(res.getBody())
	} else {
...
CREHMA.prototype.getSignatureAlgorithm = function(name){
	if(this.signatureAlgorithms[name])
		return this.signatureAlgorithms[name];
	else
		throw "Hash algorithm not available"
}
...
class CREHMARequest extends CREHMAMessage {
...
class CREHMAResponse extends CREHMAMessage{
